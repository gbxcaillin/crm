...
r.post('/admin/run-job', async (req, res) => { admin(req); const b = await readJson(req); const fn = { chat: jobs.chatDigest, digest: jobs.dailyDigest, backup: jobs.backup, market: market.refreshSecurities, bookings: bookings.sync, nurture: () => nurture.run(30, { force: true }) }[b.job]; if (!fn) throw err(400, 'Unknown job'); ok(res, { result: await fn() }); });
