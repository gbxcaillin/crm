...
async function tick() {
  ...
  try { const last = D.jobs.get.get('nurture'); if (!last || Date.now() - new Date(last.last_run).getTime() > 10 * 60e3) { mark('nurture', new Date().toISOString()); const n = await nurture.run(); mark('nurture', new Date().toISOString(), n + ' sent'); } } catch (e) { console.error('[job nurture]', e.message); }
  try { const last = D.jobs.get.get('score'); if (!last || Date.now() - new Date(last.last_run).getTime() > 5 * 60e3) { mark('score', new Date().toISOString()); const n = await scoring.autoScore(); if (n) mark('score', new Date().toISOString(), n + ' scored'); } } catch (e) { console.error('[job score]', e.message); }
